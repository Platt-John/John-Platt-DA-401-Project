...
data[data == 993] <- 0
# Replace all -9s with NA in the entire data frame
data[data == -9] <- NA
...
data <- dummy_cols(data, select_columns = c("NEWRACE2","IRWRKSTAT18"), remove_first_dummy = TRUE, remove_selected_columns = TRUE)
head(data)
#Convert illicit substance use variables into categories with codes
data$IRMJFY2 <- cut(data$IRMJFY,
                    breaks = 4,   # number of categories
                    labels = c("0-90", "90-180", "180-270", "270-360"),
                    include.lowest = TRUE)
...
data$IRHALLUCYFQ2 <- cut(data$IRHALLUCYFQ,
                    ...
                    labels = c("0-90", "90-180", "180-270", "270-360"),
                    include.lowest = TRUE)
data$IRMJFY2 <- ifelse(data$IRMJFY2 == "0-90", 1,
                  ifelse(data$IRMJFY2 == "90-180", 2,
                  ifelse(data$IRMJFY2 == "180-270", 3,
                  ifelse(data$IRMJFY2 == "270-360", 4, NA))))
data$IRCOCFY2 <- ifelse(data$IRCOCFY2 == "0-90", 1,
                  ifelse(data$IRCOCFY2 == "90-180", 2,
                  ifelse(data$IRCOCFY2 == "180-270", 3,
                  ifelse(data$IRCOCFY2 == "270-360", 4, NA))))
data$IRHALLUCYFQ2 <- ifelse(data$IRHALLUCYFQ2 == "0-90", 1,
                  ifelse(data$IRHALLUCYFQ2 == "90-180", 2,
                  ifelse(data$IRHALLUCYFQ2 == "180-270", 3,
                  ifelse(data$IRHALLUCYFQ2 == "270-360", 4, NA))))
#Install survey package
library(survey) #Library for survey design
options(survey.lonely.psu = "adjust")  #common & documented choice to adjust variance if there are single psus within stratum
#Specify survey design
...
vars <- c("IRSEX","EDUHIGHCAT","IRPRVHLT","IRHHSIZ2","INCOME","IRALCFY","IRCIGFM","IRNICVAP30N","IRALCBNG30D","SUTINPPY","IRDSTNRV12","IRIMPCONCN","IRMJFY2","IRCOCFY2","IRHALLUCYFQ2","NEWRACE2_2","NEWRACE2_3","NEWRACE2_4","NEWRACE2_5","NEWRACE2_6",
...
preds <- c("IRSEX","EDUHIGHCAT","IRPRVHLT","IRHHSIZ2","INCOME","IRALCFY","IRCIGFM",
           ...
           "IRMJFY2","IRCOCFY2","IRHALLUCYFQ2","NEWRACE2_2","NEWRACE2_3","NEWRACE2_4","NEWRACE2_5","NEWRACE2_6",
           "NEWRACE2_7","IRWRKSTAT18_2","IRWRKSTAT18_3","IRWRKSTAT18_4")
...
fit <- svyglm(
  IRSUICTHNK ~ IRMJFY2 + IRSEX + EDUHIGHCAT + IRPRVHLT + IRHHSIZ2 + IRCIGFM + IRNICVAP30N + SUTINPPY +
...
fit_int <- svyglm(
 IRSUICTHNK ~ IRSEX + EDUHIGHCAT + IRPRVHLT + IRHHSIZ2 + IRCIGFM + IRALCFY + IRALCBNG30D + IRNICVAP30N + SUTINPPY +
               IRDSTNRV12 + IRIMPCONCN + 
                NEWRACE2_2 + IRMJFY2 * NEWRACE2_3 + IRMJFY2 * NEWRACE2_4 + NEWRACE2_5 + NEWRACE2_6 + NEWRACE2_7 + IRWRKSTAT18_2 + IRWRKSTAT18_3 + IRWRKSTAT18_4,
  design = young_adults,
  family = quasibinomial()
)
#Coefficients → ORs with 95% CIs
...
regTermTest(fit_int, ~ IRMJFY2:NEWRACE2_3)
```
...
```{r see how model changes without outliers}
#File -> New Project -> Set existing GitHub Repository as working directory with data2 as a subfolder outside the repository and the NSDUH_2023.Rdata file in the subfolder
#Load in the data
load("C:/Users/John Platt/OneDrive/data2/NSDUH_2021_2023.Rdata")
#Required packages:
...
#Fit linear model
fit_linear <- svyglm(
  IRSUICTHNK ~ IRSEX + EDUHIGHCAT + IRPRVHLT + IRHHSIZ2 + INCOME +
               IRALCFY + IRCIGFM + IRNICVAP30N + IRALCBNG30D + SUTINPPY +
               IRDSTNRV12 + IRIMPCONCN +
...
#Obtain residuals
resids <- residuals(fit_linear, type = "deviance")
high_resid_idx <- which(abs(resids) > 3)
length(high_resid_idx)  #How many are flagged
df_filtered<-data[-high_resid_idx,]
#Recreate survey design with filtered dataset
des_filtered <- svydesign(
  ids = ~VEREP,
  strata = ~VESTR_C,
  weights = ~ANALWT2_C3,
  data = df_filtered,
...
#Filter survey design to young adults
young_adults3<-subset(des_filtered, CATAGE==2)
...
#Coefficients → ORs with 95% CIs
co <- coef(fit_filtered)
vc <- vcov(fit_filtered)
se <- sqrt(diag(vc))
z  <- qnorm(0.975)
...
```{r plot residuals vs fitted values for outliers adjusted PML model}
res_dev <- residuals(fit_filtered, type = "deviance")
plot(fitted(fit_filtered), res_dev, xlab="Fitted (p-hat)", ylab="Deviance residuals",
     main="Residuals vs Fitted (Deviance)", pch=19, col="steelblue")
abline(h=0, lty=2, col="red")
```

plot(predicted_lp, resids,
         xlab = "Linear Predictor",
         ylab = "Deviance Residuals",
         main = "Deviance Residuals vs. Linear Predictor")
    abline(h = 0, lty = 2, col = "grey") # Add a horizontal line at 0
